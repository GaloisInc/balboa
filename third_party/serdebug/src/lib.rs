#![allow(deprecated, bare_trait_objects)] // NOTE: added by Galois
...
impl<'a, 'b: 'a> ser::Serializer for Serializer<'a, 'b> {
    ...
    fn serialize_bytes(self, v: &[u8]) -> Result<Self::Ok, Self::Error>{
        if v.iter().all(|ch| (b' '..=b'~').contains(ch) || *ch == b'\n') {
            write!(self.0, "b{:?}", std::str::from_utf8(v).unwrap()).map_err(|e| e.into())
        } else {
            write!(self.0, "hex\"")?;
